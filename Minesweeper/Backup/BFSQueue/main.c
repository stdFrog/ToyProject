	...
	switch(iMessage){
		...
		case WM_TIMER:
			switch(wParam){
				...
				case 3:
					if(bReady == FALSE){break;}
					if(IsEmpty(Q)){KillTimer(hWnd, 3); break;}

					CurrentNode = Dequeue(Q);
					CopyBoard[CurrentNode.y][CurrentNode.x] = 1;
					for(int i=0; i<4; i++){
						xx = CurrentNode.x + dx[i];
						yy = CurrentNode.y + dy[i];

						if(xx < 0 || yy < 0 || xx >= Column || yy >= Row) {continue;}
						if(CopyBoard[yy][xx] == 0 && GameBoard[yy][xx] == 0){
							CopyBoard[yy][xx] = 1;
							Enqueue(Q, yy, xx);

							RECT ChildRect;
							GetWindowRect(hBtns[yy][xx], &ChildRect);
							ScreenToClient(hWnd, (LPPOINT)&ChildRect);
							ScreenToClient(hWnd, (LPPOINT)&ChildRect+1);
							SetWindowLongPtr(hBtns[yy][xx], GWL_STYLE, GetWindowLongPtr(hBtns[yy][xx], GWL_STYLE) & ~WS_VISIBLE);
		...
		case WM_COMMAND:
			ID = LOWORD(wParam) - IDW_BUTTON;
			{
				...
				if(GameBoard[ID / Row][ID % Column] == 0 && CopyBoard[ID / Row][ID % Column] == 0){
					if(bReady){
						Enqueue(Q, ID % Column, ID / Row);
