...
BOOL DestroyButtons(HWND** Btns, int Row);
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam){
	...
	HMENU hMenu, hPopup, hMapSizePopup;
	...
	switch(iMessage){
		case WM_CREATE:
			...
			AppendMenu(hMapSizePopup, MF_STRING, IDC_MAPSIZE+1, TEXT("10 * 10"));
			AppendMenu(hMapSizePopup, MF_STRING | MF_CHECKED, IDC_MAPSIZE+2, TEXT("20 * 20"));
			AppendMenu(hMapSizePopup, MF_STRING, IDC_MAPSIZE+3, TEXT("30 * 30"));
			AppendMenu(hMapSizePopup, MF_STRING, IDC_MAPSIZE+4, TEXT("40 * 40"));
			AppendMenu(hMapSizePopup, MF_STRING, IDC_MAPSIZE+5, TEXT("50 * 50"));
			SetMenu(hWnd, hMenu);
			SelectMenuNumber = 1;
			PopupNumber = 2;
			Start = 0;
		...
		case WM_COMMAND:
			if(LOWORD(wParam) == IDC_MENU){
				...
				if(LOWORD(wParam) != IDC_MENU + 1){
					...
					Row = Column = (LOWORD(wParam) - IDC_MAPSIZE) * 10;
					ButtonLayout.y = Row;
					ButtonLayout.x = Column;
					ButtonLayout.Width = ButtonLayout.Height = DEFAULT_BTN_SIZE;
					GameStateLayout.x = GameStateLayout.y = 0;
...
LRESULT CALLBACK GameStateWndProc(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam){
	...
	switch(iMessage){
		case WM_CREATE:
			hStateButton = CreateWindow(TEXT("button"), NULL, WS_VISIBLE | WS_BORDER | WS_CHILD | BS_PUSHBUTTON | BS_BITMAP, 0,0,0,0, GetParent(hWnd), (HMENU)NULL, GetModuleHandle(NULL), NULL);
			return 0;
...
BOOL CreateButtons(HWND*** Btns, int Row, int Column){
	if((*Btns) != NULL){
		if(!DestroyButtons((*Btns), sizeof((*Btns)) / sizeof((*Btns)[0]))){
			return FALSE;
		}
	}
