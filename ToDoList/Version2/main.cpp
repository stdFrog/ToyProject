...
#define KEY_PATH_MODE		L"Software\\SicSoft\\InitInfo\\LastMode"
...
class MainWindow : public BaseWindow<MainWindow> {
    ...
    MSGMAP MainMsg[_nMsg] = {
		...
		{WM_GETMINMAXINFO, &MainWindow::OnGetMinMaxInfo},
	...
	iModeInfo iModePosition = {0,0,0,0};
...
private:
	...
	BOOL InitTray();
	BOOL DeleteTray();
	BOOL GetTodaySchedule();
	BOOL CreateScheduleEdit();
...
BOOL MainWindow::LoadFile(){
	...
	if(Header.Version == 200 && wcscmp(Header.lpszHeader, L"ToDoList Data File") == 0){
		...
		case IDM_POPUP1:
			...
			if(bMode == TRUE){
				...
				for(int i=0; i<nControls; i++){
					ShowWindow(hControls[i], SW_SHOW);
				}
				SetWindowPos(_hWnd, NULL, PrevWndRect.left, PrevWndRect.top, PrevWndRect.right - PrevWndRect.left, PrevWndRect.bottom - PrevWndRect.top, SWP_NOZORDER);
...
LRESULT MainWindow::OnDestroy(WPARAM wParam, LPARAM lParam) {
	...
	WriteRegistryData(HKEY_CURRENT_USER, KEY_PATH_MODE, KEY_WRITE, L"LastMode", REG_DWORD, &bMode, sizeof(bMode));
	if(bMode == FALSE){ SavePosition(_hWnd, HKEY_CURRENT_USER, KEY_PATH_POSITION); }
	DeleteTray();
...
LRESULT MainWindow::OnGetMinMaxInfo(WPARAM wParam, LPARAM lParam){
	LPMINMAXINFO pmmi;
	RECT wrt;

	if(bMode == TRUE){
		pmmi = (LPMINMAXINFO)lParam;
		pmmi->ptMinTrackSize.x	= iModePosition.Width;
		pmmi->ptMinTrackSize.y	= iModePosition.Height;
		pmmi->ptMaxTrackSize.x	= iModePosition.Width;
		pmmi->ptMaxTrackSize.y	= iModePosition.Height;
		pmmi->ptMaxSize.x		= iModePosition.Width;
		pmmi->ptMaxSize.y		= iModePosition.Height;

		GetWindowRect(_hWnd, &wrt);
		pmmi->ptMaxPosition.x	= wrt.left;
		pmmi->ptMaxPosition.y	= wrt.top;
	}
...
LRESULT MainWindow::OnActivateApp(WPARAM wParam, LPARAM lParam){
	...
	if(wParam == TRUE){
		if(bInit){
			bInit = FALSE;
			dwType = ReadRegistryData(HKEY_CURRENT_USER, KEY_PATH_MODE, KEY_READ, L"LastMode", &bMode, sizeof(bMode));
			if(dwType == REG_NONE){ bMode = FALSE; }
			...
			if(bMode == TRUE){
				...
				dwType = ReadRegistryData(HKEY_CURRENT_USER, KEY_PATH_POSITION, KEY_READ, L"Right", &PrevWndRect.right, sizeof(LONG));
				dwType = ReadRegistryData(HKEY_CURRENT_USER, KEY_PATH_POSITION, KEY_READ, L"Bottom", &PrevWndRect.bottom, sizeof(LONG));
			}else{
				CheckMenuItem(hPopupTray, IDM_POPUP1, MF_BYCOMMAND | MF_UNCHECKED);
				GetWindowRect(_hWnd, &PrevWndRect);
			}
		}
	}
...
BOOL MainWindow::GetTodaySchedule(){
	// TODO: 일반 모드에서 창이 숨겨져 있을 때 TodaySchedule 활성시 화면에 창이 일시적으로 노출되는 현상
	if(!IsWindowVisible(_hWnd)){ ShowWindow(_hWnd, SW_SHOW); }
	...
	for(i=0; i<nControls; i++){
		ShowWindow(hControls[i], SW_HIDE);
	}
	...
	iModePosition.Width		= crt.right - crt.left;
	iModePosition.Height	= crt.bottom - crt.top;
	iModePosition.x			= ScreenWidth - iModePosition.Width;
	iModePosition.y			= ScreenHeight - iModePosition.Height;
	SetWindowPos(_hWnd, NULL, iModePosition.x, iModePosition.y - GetSystemMetrics(SM_CYCAPTION), iModePosition.Width, iModePosition.Height, SWP_NOZORDER);

	HDC hMemDC = CreateCompatibleDC(hdc);
	HBITMAP hBitmap = CreateCompatibleBitmap(hdc, iModePosition.Width, iModePosition.Height);
	HGDIOBJ hOld = SelectObject(hMemDC, hBitmap);
	FillRect(hMemDC, &crt, GetSysColorBrush(COLOR_WINDOW));

	for(i=0; i<j; i++){
		TextOut(hMemDC, 0, i * Metric.tmHeight, lpszCopy[i], wcslen(lpszCopy[i]));
	}

	BitBlt(hdc, 0,0, iModePosition.Width, iModePosition.Height, hMemDC, 0,0, SRCCOPY);
	...
	bMode = TRUE;
	return (BOOL)(CheckMenuItem(hPopupTray, IDM_POPUP1, MF_BYCOMMAND | MF_CHECKED) != -1);
}

BOOL MainWindow::CreateScheduleEdit(){
	return TRUE;
}
